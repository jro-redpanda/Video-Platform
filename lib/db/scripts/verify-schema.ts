...
const { Client } = pg;
const tables = ["plans","organizations","organization_customization","users","sessions","accounts","verifications","permission_groups","permissions","group_permissions","memberships","invitations","provider_accounts","provider_tenant_spaces","organization_entitlement_overrides","webhook_events","embed_generation_outbox","audit_logs","folders","videos","video_embeds","video_analytics_rollups","playback_events","thumbnail_upload_intents","object_cleanup_outbox","organization_billing","billing_operations","billing_event_receipts"];
...
const rls = ["organization_customization","permission_groups","memberships","invitations","folders","videos","thumbnail_upload_intents","video_analytics_rollups","playback_events","provider_tenant_spaces","organization_entitlement_overrides","audit_logs","organization_billing","billing_operations","billing_event_receipts"];
async function main() {
  ...
  try {
    ...
    const final = await catalogFingerprint(client, "billing");
    if (final.hash !== EXPECTED_CATALOG_HASHES.billing) throw new Error(`billing catalog fingerprint mismatch: ${final.hash}`);
