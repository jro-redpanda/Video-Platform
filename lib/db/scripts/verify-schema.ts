...
const rls = ["organization_customization","permission_groups","memberships","invitations","folders","videos","thumbnail_upload_intents","video_analytics_rollups","playback_events","provider_tenant_spaces","organization_entitlement_overrides","audit_logs","organization_billing","billing_operations","billing_event_receipts","analytics_dirty_days","analytics_rate_windows","analytics_playback_sessions"];
async function main() {
  ...
  try {
    ...
    const final = await catalogFingerprint(client, "auditExport");
    if (final.hash !== EXPECTED_CATALOG_HASHES.auditExport) throw new Error(`audit export catalog fingerprint mismatch: ${final.hash}`);
