...
async function main() {
  ...
  try {
    ...
    const final = await catalogFingerprint(client, "g3Hardening");
    if (EXPECTED_CATALOG_HASHES.g3Hardening.startsWith("REPLACE_")) throw new Error("g3Hardening catalog hash is unset; derive it from an isolated or rollback-only migrated database");
    if (final.hash !== EXPECTED_CATALOG_HASHES.g3Hardening) throw new Error(`G3 catalog fingerprint mismatch: ${final.hash}`);
